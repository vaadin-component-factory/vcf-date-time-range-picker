...
class MonthCalendarElement extends ThemableMixin(GestureEventListeners(PolymerElement)) {
        ...
        <div id="days">
          <template is="dom-repeat" items="[[_days]]">
            <div
                part="date"
                today$="[[_isToday(item)]]"
                selected$="[[_dateBetween(item, selectedStartDate, selectedEndDate)]]"
  ...
  static get properties() {
    return {
      ...
      /**
       * A `Date` object for the currently selected start date.
       */
      selectedStartDate: {
        type: Date,
        notify: true
      ...
      /**
       * A `Date` object for the currently selected end date.
       */
      selectedEndDate: {
        type: Date,
        notify: true
  ...
  _dateBetween(date1, startDate, endDate) {
    return dateBetween(date1, startDate, endDate);
  }
  ...
  _handleTap(e) {
    if (!this.ignoreTaps && !this._notTapping && e.target.date && !e.target.hasAttribute('disabled')) {
      if (this.selectingStartDate) {
        this.selectedStartDate = e.target.date;
      } else {
        this.selectedEndDate = e.target.date;
      }
